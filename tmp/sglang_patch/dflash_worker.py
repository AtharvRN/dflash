...
from sglang.srt.speculative.dflash_dynamic_block import (
    DFlashDynamicBlockController,
    parse_dflash_dynamic_block_arms,
)
from sglang.srt.speculative.dflash_survival_block import (
    DFlashSurvivalBatchController,
    parse_arms as parse_dflash_survival_arms,
)
...
class DFlashWorker:
    ...
    def __init__(
        ...
        self._last_logged_dynamic_block_size: Optional[int] = None
        self._profile_cycles = os.environ.get("SGLANG_DFLASH_PROFILE_CYCLES", "0").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        self._profile_log_path = os.environ.get("SGLANG_DFLASH_PROFILE_LOG")
        self._profile_log_every = max(
            1, int(os.environ.get("SGLANG_DFLASH_PROFILE_LOG_EVERY", "100"))
        )
        self._profile_cycle_idx = 0
        self._active_profile_record: Optional[dict] = None
        self._active_profile_t0: Optional[float] = None
        if self._profile_cycles and self._profile_log_path and self.tp_rank == 0:
            Path(self._profile_log_path).parent.mkdir(parents=True, exist_ok=True)
            with open(self._profile_log_path, "a", encoding="utf-8") as f:
                f.write("")
        ...
        self.current_block_size = int(self.block_size)
        self.dynamic_block_controller: Optional[DFlashDynamicBlockController] = None
        self.survival_block_controller: Optional[DFlashSurvivalBatchController] = None
        if server_args.speculative_dflash_dynamic_block_size:
            dynamic_arms = parse_dflash_dynamic_block_arms(
                server_args.speculative_dflash_dynamic_block_arms
            )
            self.dynamic_block_controller = DFlashDynamicBlockController(
                arms=dynamic_arms,
                initial_block_size=int(self.block_size),
                target_accept_ratio=server_args.speculative_dflash_dynamic_target_ratio,
                ema_alpha=server_args.speculative_dflash_dynamic_ema_alpha,
                warmup_batches=server_args.speculative_dflash_dynamic_warmup_batches,
                update_interval=server_args.speculative_dflash_dynamic_update_interval,
            )
            self.current_block_size = (
                self.dynamic_block_controller.current_block_size()
            )
        survival_checkpoint = os.environ.get("SGLANG_DFLASH_SURVIVAL_POLICY_CHECKPOINT")
        if survival_checkpoint:
            survival_arms = parse_dflash_survival_arms(
                os.environ.get(
                    "SGLANG_DFLASH_SURVIVAL_ARMS",
                    server_args.speculative_dflash_dynamic_block_arms,
                )
            )
            survival_alpha = float(os.environ.get("SGLANG_DFLASH_SURVIVAL_ALPHA", "0.90"))
            self.survival_block_controller = DFlashSurvivalBatchController(
                checkpoint_path=survival_checkpoint,
                arms=survival_arms,
                alpha=survival_alpha,
                device=self.device,
                draft_model=self.draft_model,
                monotonicize_probs=True,
                internal_feature_seed=int(os.environ.get("SGLANG_DFLASH_SURVIVAL_FEATURE_SEED", "0")),
            )
            self.current_block_size = self.survival_block_controller.last_block_size
        ...
        if self.tp_rank == 0:
            ...
            if self.dynamic_block_controller is not None:
                logger.info(
                    "DFLASH dynamic block controller enabled. "
                    "arms=%s, initial_block_size=%s, max_block_size=%s",
                    list(self.dynamic_block_controller.arms),
                    self.current_block_size,
                    self.block_size,
                )
            if self.survival_block_controller is not None:
                logger.info(
                    "DFLASH survival block controller enabled. "
                    "arms=%s, alpha=%.3f, initial_block_size=%s, max_block_size=%s",
                    list(self.survival_block_controller.arms),
                    self.survival_block_controller.alpha,
                    self.current_block_size,
                    self.block_size,
                )
            logger.info(
            ...
            if self._profile_cycles:
                logger.info(
                    "DFLASH per-cycle profiling enabled. log_path=%s log_every=%s",
                    self._profile_log_path,
                    self._profile_log_every,
                )
    ...
    def _profile_timestamp(self) -> float:
        if self._profile_cycles and is_cuda():
            torch.cuda.synchronize()
        return time.perf_counter()

    def _profile_start_cycle(self, *, batch: ScheduleBatch, draft_input: DFlashDraftInput) -> None:
        if not self._profile_cycles or self.tp_rank != 0:
            return
        self._profile_cycle_idx += 1
        self._active_profile_t0 = self._profile_timestamp()
        self._active_profile_record = {
            "cycle_idx": self._profile_cycle_idx,
            "batch_size": int(batch.batch_size()),
            "max_block_size": int(self.block_size),
            "current_block_size_before": int(self.current_block_size),
            "num_target_hidden_tokens": int(draft_input.target_hidden.shape[0])
            if draft_input.target_hidden is not None
            else 0,
            "ctx_lens_sum": int(draft_input.ctx_lens.sum().item()),
            "timings_ms": {},
        }

    def _profile_mark(self, name: str) -> None:
        if (
            not self._profile_cycles
            or self.tp_rank != 0
            or self._active_profile_record is None
            or self._active_profile_t0 is None
        ):
            return
        now = self._profile_timestamp()
        self._active_profile_record["timings_ms"][name] = (
            now - self._active_profile_t0
        ) * 1000.0
        self._active_profile_t0 = now

    def _profile_set(self, **values) -> None:
        if (
            not self._profile_cycles
            or self.tp_rank != 0
            or self._active_profile_record is None
            ...
            return
        self._active_profile_record.update(values)

    def _profile_finish_cycle(
        self,
        *,
        ...
        if (
            ...
            return
        self._profile_mark("cycle_finalize")
        record = self._active_profile_record
        timings = record.get("timings_ms", {})
        record["total_profiled_ms"] = float(sum(timings.values()))
        record["num_correct_drafts_sum"] = int(sum(num_correct_drafts_per_req_cpu))
        record["num_correct_drafts_per_req"] = [
            int(x) for x in num_correct_drafts_per_req_cpu
        ]
        record["commit_lens_sum"] = int(commit_lens.sum().item())
        record["commit_lens_mean"] = (
            float(commit_lens.float().mean().item()) if commit_lens.numel() else 0.0
        )
        if self._profile_log_path:
            with open(self._profile_log_path, "a", encoding="utf-8") as f:
                f.write(json.dumps(record, sort_keys=True) + "\n")
        if self._profile_cycle_idx % self._profile_log_every == 0:
            logger.info("DFLASH cycle profile: %s", json.dumps(record, sort_keys=True))
        self._active_profile_record = None
        self._active_profile_t0 = None
    ...
    def on_verify_complete_cpu(
        ...
        if self.dynamic_block_controller is None:
            return

        new_block_size = self.dynamic_block_controller.on_verify_complete(
            num_correct_drafts_per_req, batch_size=batch_size
        )
        self.current_block_size = self.dynamic_block_controller.current_block_size()
        if new_block_size is not None and self.tp_rank == 0:
            logger.info(
                "DFLASH dynamic block size updated to %s. "
                "accepted_drafts_ema=%.3f, accept_ratio_ema=%.3f, "
                "full_accept_rate_ema=%.3f",
                new_block_size,
                self.dynamic_block_controller.last_change_accepted_drafts_ema or 0.0,
                self.dynamic_block_controller.last_change_accept_ratio_ema or 0.0,
                self.dynamic_block_controller.last_change_full_accept_rate_ema or 0.0,
            )

    def _get_runtime_block_size(self, batch_size: int) -> int:
        del batch_size
        if self.survival_block_controller is not None:
            runtime_block_size = int(self.survival_block_controller.last_block_size)
            if runtime_block_size < 2 or runtime_block_size > int(self.block_size):
                raise RuntimeError(
                    "Invalid DFLASH survival runtime block size: "
                    f"runtime={runtime_block_size}, max={self.block_size}."
                )
            self.current_block_size = runtime_block_size
            if (
                self.tp_rank == 0
                and self._last_logged_dynamic_block_size != runtime_block_size
            ):
                logger.info(
                    "DFLASH survival using runtime block size %s (max=%s, batch_size=%s, mean_expected_by_arm=%s).",
                    runtime_block_size,
                    self.block_size,
                    self.survival_block_controller.last_batch_size,
                    [round(x, 3) for x in self.survival_block_controller.last_mean_expected_by_arm],
                )
                self._last_logged_dynamic_block_size = runtime_block_size
            return runtime_block_size
        if self.dynamic_block_controller is None:
            return int(self.block_size)
        runtime_block_size = int(self.dynamic_block_controller.current_block_size())
        if runtime_block_size < 2 or runtime_block_size > int(self.block_size):
            raise RuntimeError(
                "Invalid DFLASH runtime block size: "
                f"runtime={runtime_block_size}, max={self.block_size}."
            )
        self.current_block_size = runtime_block_size
        if (
            ...
            logger.info(
                "DFLASH using runtime block size %s (max=%s).",
                runtime_block_size,
                self.block_size,
            )
            self._last_logged_dynamic_block_size = runtime_block_size
    ...
    def _prepare_for_speculative_decoding(
        ...
        bs = batch.batch_size()
        req_ids = (
            [str(req.rid) for req in batch.reqs]
            if (self.survival_block_controller is not None or self._profile_cycles)
            else None
        )

        # --- 1) Append any newly committed tokens into the draft KV cache.
        self._append_target_hidden_to_draft_kv(batch, draft_input)
        self._profile_mark("draft_kv_append_pre")
        if self.survival_block_controller is not None:
            assert req_ids is not None
            self.survival_block_controller.observe_context(
                draft_model=self.draft_model,
                req_ids=req_ids,
                target_hidden=draft_input.target_hidden,
                ctx_lens=draft_input.ctx_lens,
            )
            self.survival_block_controller.select_block_size(req_ids)
        self._profile_mark("survival_policy")
        ...
        runtime_block_size = self._get_runtime_block_size(bs)
        self._profile_set(
            runtime_block_size=int(runtime_block_size),
        )
        self._profile_mark("block_select_and_buffer")
        ...
        try:
            ...
            input_embeds = noise_embedding.reshape(-1, noise_embedding.shape[-1])
            ...
            positions_2d = self._draft_block_positions_buf[
            ...
            torch.add(block_start, int(runtime_block_size), out=block_end)
            ...
            if self.page_size == 1:
                ...
                block_end_cpu = seq_lens_cpu + int(runtime_block_size)
            ...
            if block_cache_loc is None:
                raise RuntimeError(
                    f"DFLASH draft OOM when allocating {bs * runtime_block_size} block tokens."
                )
            ...
            self._profile_mark("draft_setup")
            with torch.inference_mode():
                draft_logits_output = self.draft_model_runner.forward(
                    forward_batch
                ).logits_output
            self._profile_mark("draft_forward")
        finally:
        ...
        positions = positions_2d.reshape(-1)
        self._profile_mark("draft_sample")
        ...
        self._profile_mark("verify_prepare")
    ...
    def forward_batch_generation(
        ...
        self._profile_start_cycle(batch=batch, draft_input=draft_input)
        ...
        self._profile_mark("target_verify_forward")
        ...
        self._profile_mark("accept_verify")
        self.on_verify_complete_cpu(
            num_correct_drafts_per_req_cpu,
            batch_size=len(num_correct_drafts_per_req_cpu),
        )
        self._profile_mark("controller_update")
        ...
        self._profile_mark("mamba_commit")
        ...
        self._append_target_hidden_to_draft_kv(batch, draft_input)
        self._profile_mark("draft_kv_append_post")
        batch.spec_info = draft_input
        batch.forward_mode = ForwardMode.DECODE

        num_correct_drafts = sum(num_correct_drafts_per_req_cpu)
        self._profile_finish_cycle(
