...
from dflash.policy import DFlashOracleArmBlockPolicy
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Online C=1 smoke-test for a DFlash oracle-arm block policy.")
    ...
    parser.add_argument("--policy-threshold", type=float, default=0.67)
    parser.add_argument("--dataset", default="math500")
    parser.add_argument("--max-samples", type=int, default=8)
    parser.add_argument("--max-new-tokens", type=int, default=256)
    parser.add_argument("--fixed-block-size", type=int, default=12)
    ...
    parser.add_argument("--compare-output-ids", action="store_true")
...
def main() -> None:
    ...
    policy = DFlashOracleArmBlockPolicy(
    ...
    fixed_responses = []
    dynamic_responses = []
    num_mismatched_outputs = 0

    for instance in tqdm(dataset, desc="oracle dynamic decode"):
        messages = []
        for user_content in instance["turns"]:
            ...
            dynamic_responses.append(dynamic)

            if args.compare_output_ids and not torch.equal(fixed.output_ids, dynamic.output_ids):
                num_mismatched_outputs += 1

            generated_ids = dynamic.output_ids[0, dynamic.num_input_tokens :]
    ...
    payload = {
        ...
        "temperature": args.temperature,
        "num_responses": len(dynamic_responses),
        "num_mismatched_outputs": num_mismatched_outputs,
        "fixed": _fixed_metrics(fixed_responses, args.fixed_block_size),
        "dynamic": _dynamic_metrics(dynamic_responses),
    }
